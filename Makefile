...
all: fmt lint $(BIN) ; $(info $(M) building executable…) @ ## Build program binary
	$Q $(GO) build \
		-tags release \
		-ldflags '-X gitlab.com/leucos/toji/cmd.Version=$(VERSION) -X gitlab.com/leucos/toji/cmd.BuildDate=$(DATE)' \
		-o $(BIN)/$(PACKAGE)
...
linux: fmt lint $(BIN) ; $(info $(M) building static executable for Linux……) @ ## Build program binary
	$Q env GOOS=linux GOARCH=amd64 CGO_ENABLED=0 $(GO) build \
		-tags release -a \
		-ldflags '-w -extldflags "-static" -X gitlab.com/leucos/toji/cmd.Version=$(VERSION) -X gitlab.com/leucos/toji/cmd.BuildDate=$(DATE)' \
